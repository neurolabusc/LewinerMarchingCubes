...
void MarchingCubes::writeNIfTI(const char *fn) {
  TNIFTI hdr;
  ...
  int nvox = _size_x * _size_y * _size_z;
  fwrite(_data, nvox * sizeof(uchar), 1, fp);
  fclose(fp);
}

void MarchingCubes::writePLY(const char *fn) {
  ...
  for (int i = 0; i < npt; i++) {
    pts32[i].x = _vertices[i].x;
    pts32[i].y = _vertices[i].y;
    pts32[i].z = _vertices[i].z;
  }
  ...
  for (int i = 0; i < ntri; i++) {
    tris4[i].n = 3;
    tris4[i].x = _triangles[i].v3;
    tris4[i].y = _triangles[i].v2;
    tris4[i].z = _triangles[i].v1;
  }
...
uchar * readNIFTI8(const char *fn, int *dim) {
  FILE* fp = fopen (fn, "r");
  TNIFTI hdr;
  size_t sz = fread(&hdr, sizeof(hdr), 1, fp);
  if ((hdr.datatype != 2) || (hdr.sizeof_hdr != 348) || (hdr.vox_offset < 352)) {
    fclose(fp);
    printf("Error: only able to read uncompressed .nii uint8 NIfTI images of native endian.\n");
    return NULL;
  }
  dim[0] = hdr.dim[1];
  dim[1] = hdr.dim[2];
  ...
  uchar * img8 = (uchar *) malloc(nvox*sizeof(uchar));
  sz = fread(img8, nvox, 1, fp);
  fclose(fp);
  return img8;
}
...
void self_test(const char *fn, float isolevel, int originalMC) {
  ...
  mc.init_all() ;
  mc.set_ext_data(img8);
  printf("input voxels: %d %d %d\n", dim[0], dim[1], dim[2]);
  mc.set_method(originalMC );
  double startTime = clockMsec();
  mc.run(isolevel);
  printf("output mesh vert: %d tris: %d ms: %ld\n", mc.nverts(), mc.ntrigs(),  timediff(startTime, clockMsec()));
  #define MAX_BUF 1024
  char meshfn[MAX_BUF];
  int len = snprintf (meshfn, MAX_BUF-1, "./%d.ply", (int)isolevel);
  mc.writePLY(meshfn);
  mc.clean_all();
  free(img8);
}

//c++ -std=c++11 -O3 -DMC_SELF_TEST marchingcubes.cpp -o mc; ./mc bet.nii 128  
int main(int argc, char **argv) {
  ...
  printf("For help, run without arguments\n");
  exit(EXIT_SUCCESS);
}
#endif // MC_SELF_TEST
